#!/bin/ksh
#source https://www.pythian.com/blog/oracle-instance-memory-usage/
...
if [ -n "$pids" ]; then
export countcon=`echo "$pids"|wc -l`

if [ "`uname -a|cut -f1 -d' '`" = "Linux" ]; then
if [ $dev_shm_count -gt 0 ]; then
export tconprivsz=$(pmap -x `echo "$pids"`|grep " rw"|grep -Evf shm_addresses|awk '{total +=$2};END {print total}')
else
export tconprivsz=$(pmap -x `echo "$pids"`|grep " rw"|grep -Ev "shmid|deleted"|awk '{total +=$2};END {print total}')
fi
else
export tconprivsz=$(pmap -x `echo "$pids"`|grep " rw"|grep -v "shmid"|awk '{total +=$2};END {print total}')
fi
